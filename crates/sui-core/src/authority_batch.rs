...
impl crate::authority::AuthorityState {
    ...
    pub async fn handle_batch_streaming(
        ...
        // We could not even find the 0 batch, return error
        if batch.is_none() {
        ...
        struct BatchStreamingLocals<GuardT> {
            _guard: GuardT,
            // Next tx with sequence number that should be read from db
            next_tx_seq: TxSequenceNumber,
            // Next batch to be read from db should have its next_sequence_number be at least this much
            next_batch_seq: TxSequenceNumber,
            // Read txn from db if this is true or batch otherwise
            read_next_tx: bool,
            // If none, no more batch will be published in the stream
            // i.e. only trailing transactions will be published
        ...
        let stream1 = stream::unfold(local_state, move |mut local_state| async move {
            loop {
                if local_state.pending_batch.is_some() && !local_state.read_next_tx {
                    // We already publihsed all txns for this batch, now is the time to
                    // publish the batch
                ...
                } else if local_state.read_next_tx {
                    ...
                    if let Some((seq, digest)) = tx {
                        ...
                        if local_state.next_tx_seq >= (local_state.next_batch_seq - 1) {
                            // We read all txns for this batch
                            local_state.read_next_tx = false;
                        }
                        return Some((
